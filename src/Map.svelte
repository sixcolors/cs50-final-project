<script lang="ts">
          ...
          .bindPopup(
            ...
            ${
              typeof distance === "number"
                ? `<p>${Math.round(distance / 1000)} km from ${userLocationPlaceName}.</p>`
                : ""
            }
    ...
    onMount(() => {
      ...
      // download data
      fetch("https://cwfis.cfs.nrcan.gc.ca/downloads/activefires/activefires.csv")
  ...
  <div class="map">
    <LeafletMap bind:this={map} options={mapOptions}>
      ...
      <TileLayer bind:this={wmsFireTileLayer} wms={true} url={wmsFireTileUrl} options={wmsFireTileLayerOptions} />
    </LeafletMap>
  </div>
  ...
  .map {
    ...
    padding: 1rem;
    height: calc(
       100vh - env(safe-area-inset-top) - env(safe-area-inset-bottom) - 3rem
    );
  }
